...
class HomeController extends Controller
{
    ...
    public function category($id)
    {
        ...
        if (!empty($data)) {
            ...
            $pro = Product::whereIn('cat_id', $cat_id)->orderBy('created_at', 'desc')->get()->toArray();
            return view('font-end/home.category', compact('Cate','cat_main'))->with(['pro_cat' => $pro]);
        } else {
            $pro = Product::where('cat_id', $id)->orderBy('created_at', 'desc')->get()->toArray();
            return view('font-end/home.category', compact('Cate', 'cat_main'))->with(['pro_cat' => $pro]);
        }
    }
    ...
    public function search(Request $request)
    {
        if($request->get('query'))
        {
            ...
            // search product
            $pro_search= Product::select('avatar', 'name', 'id', 'price', 'discount','created_at')
