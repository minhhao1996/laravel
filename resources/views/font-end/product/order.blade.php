                                    ...
                                    <div class="step-preview clearfix">
                                        ...
                                        @if(!isset($profile))
                                            <div class="form-block ng-scope" ng-if="CustomerId<=0">
                                                <div class="user-login"><a href="{{url('resgister')}}">Đăng ký tài khoản
                                                        mua
                                                        hàng</a><a
                                        ...
                                        @else
                                            ...
                                            <div class="form-block ng-hide">
                                                <div class="form-group"><input class="form-control " value="{{$profile['name']}}" name="name"
                                                      placeholder="Họ &amp; Tên" type="text">
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    <input name="email" class="form-control " value="{{$profile['email']}}" disabled
                                                    placeholder="Email của bạn" type="email">
                                                </div>
                                                <div class="form-group"><input name="address" class="form-control " value="{{$profile['address']}}"
                                                    placeholder="Địa chỉ của bạn" type="text">
                                                </div>
                                                <div class="form-group">
                                                    <select name="country" id="province_order" class="form-control "
                                                            required>
                                                        ...
                                                        @foreach($city as $country)
                                                            <option value="{{ $country['id']}}"
                                                                    <?php if ($country['id'] == $profile['city_id']) echo 'selected';?> label="{{ $country['name'] }}">
                                                                {{ $country['name'] }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="form-group">
                                                    @if(!empty($district))
                                                        <select class="form-control " name="district"
                                                                id="district_order" required>
                                                            @foreach($district as $a)
                                                                <option value="{{$a['id']}}" <?php if ($a['id'] == $profile['county_id']) echo 'selected';?>>{{$a['name']}}</option>
                                                            @endforeach
                                                        </select>
                                                    @else
                                                        <select class="form-control " name="district" id="district_order">
                                                ...
                                                <textarea class="form-control " rows="10" name="about"
                                                          placeholder="Ghi chú đơn hàng"></textarea>
                                            </div>
                                    ...
                                    <div class="step-preview clearfix">
                                        ...
                                        <div class="form-group ">
                                            <select class="form-control " id="transport" required>
                                                <option value="">Vui lòng chọn vận chuyển</option>
                                    ...
                                    <div class="step-preview">
                                        <div class="cart-info">
                                                    ...
                                                    <div class="cart-item clearfix ng-scope">
                                                           <span class="image pull-left" style="margin-right:10px;">
                                                               <a href="{{url('details',$row['id'])}}">
                                                                    <img src="{{asset('upload/product/avatar')}}/{{$row['attributes']['img']}}" class="img-responsive">
                                                             </a>
                                                          </span>
                                                    <div class="product-info pull-left">
                                                        <span class="product-name">
                                                               <a href="{{url('details',$row['id'])}}" class="ng-binding">{{$row['name']}}</a> x
                                                            <span class="ng-binding">{{$row['quantity']}}</span>
                                                         </span>
                                                    </div>
                                            ...
                                            <div class="shiping-price">
                                                Phí vận chuyển <label class="shiping" >Vui lòng chọn ship</label>
                                            </div>
                                            ...
                                            <div class="total-payment">
                                                Thanh toán <span class="total-order"> {{number_format($total)}} ₫</span>
                                            </div>
                        ...
                        $(document).ready(function ($) {
                            $('#province_order').on('change', function (e) {
                                ...
                                $.ajax({
                                    ...
                                    let html = '<option value="" selected >' +
                            ...
                            $('#transport').on('change', function (e){
                                let ship = $("#transport").val();
                                let token = $("input[name='_token']").val();
                                $.ajax({
                                    type: 'get', // phương thức gửi
                                    url: '{{route('get-shipping')}}', //tới route mà chúng ta đã định nghĩa ở trên.
                                    data: {
                                        ship: ship,
                                        _token: token
                                    },
                                      ...
                                      let  ship = res['ship']+'đ';
                                       let total_order =res['total']+'đ';
                                      $('.shiping').html(ship);
                                      $('.total-order').html(total_order);
                                });
                            });
                        });
                    </script>
